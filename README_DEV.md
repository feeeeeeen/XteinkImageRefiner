...
Xteink X4 電子書籍リーダー向けに、漫画・自炊画像を最適化・変換するデスクトップアプリケーション。
...
| 3 | リサイズ（リサイズなし・X4プリセット・最大幅/高さ・配置・シャープ） | 常時（リサイズなし時はX4・幅・高さ・配置・シャープ・自動回転を無効化。X4チェック時は最大幅・高さを無効化）|
...
- 有効時に無効化される UI: X4プリセット、最大幅/高さ、配置（ラベル含む）、シャープ（ラベル含む）、自動回転。
...
> **読み込み順序の依存関係**: `no_resize` → `width`/`height` → `x4_preset` の順で読み込む。`on_x4_changed` 内で `no_resize_check` の状態を参照するため。
...
6. **UI 改善**:
   ...
   - Xteink X4 プリセット（480×800固定）チェックボックス追加。
...
18. **リサイズなしモード**:
    ...
    - 関連 UI（X4プリセット・幅・高さ・配置・シャープ・自動回転）をグレーアウトで無効化（ラベル含む）。
...
- **プリセット機能**: 端末ごとの解像度・ビット数・ディザ設定を保存・呼び出し。
