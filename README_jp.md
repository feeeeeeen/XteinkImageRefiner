...
- **端末プリセット**: Xteink X3 (528x792) / X4 (480x800) / M5Paper S3 (540x960) の解像度プリセットを内蔵
- **フォルダごとに保存**: バッチ処理用モード — 入力フォルダごとに個別の出力ファイルを生成（連番付与）
- **リサイズなしモード**: 画像のソート・リネーム・パッキング用途に対応
- **メタデータ自動取得**: `[著者名] タイトル` 形式のフォルダ/ZIP 名から自動抽出。編集ON/OFF切替で入力名を直接出力名に使用可能
...
- 自動分割に「適用対象」（両方/左右のみ/上下のみ）と「左右分割の順序」（左→右/右→左）のトグルボタンを追加（右綴じ漫画対応）
- 分割画像のファイル名 suffix を `_1` / `_2` に統一（旧 `_left/_right/_top/_bot`）
- プレビューの分割切替ボタンを「1番目 / 2番目」固定表示に変更し、選択中のボタンをハイライト。分割対象外画像表示時は無効化
- 自動分割の適用対象を切り替えた際、手動調整したクロップ矩形が保持されるように改善（旧: 全消去されていた）
- 余白自動検出スレッドが自動分割の適用対象設定を考慮するように改善（不要な検出処理を削減）

### 2026-05-08

- M5Paper S3 (540x960) プリセットを追加
- 「フォルダごとに保存」モードを追加（ソースフォルダごとに別の出力ファイルを生成、自動連番付与）
- 出力名グループに「編集する」チェックボックスを追加（OFFでソース名をそのまま出力名に使用）
- ソート時にソースラベル単位のグループ化を維持（ソースラベル→ファイル名の順でソート）
- メタデータ自動取得を最初のソース読み込み時のみに変更（追加読み込み時は上書きしない）
- 出力名にWindows無効文字のサニタイズ処理を追加
- 同名フォルダを別パスから読み込んだ場合も内部のソースIDで区別
